...
import { Activity, Database, GitBranch, Search, Users, FileText, CheckCircle, BookOpen, Menu, X } from 'lucide-react';
...
function Sidebar({ isOpen, setIsOpen }) {
  return (
    <>
      ...
      {/* Sidebar Content */}
      <div className={`fixed lg:sticky top-0 left-0 h-screen w-72 bg-[#00071b] text-white flex flex-col z-50 transition-transform duration-300 ease-in-out border-r border-[#1a365d] shadow-2xl lg:shadow-none lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-[#7887ab] mt-1 tracking-widest uppercase font-bold">Data Science Capstone</p>
          </div>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
          <NavLink to="/" icon={Activity} onClick={() => setIsOpen(false)}>Dashboard</NavLink>
          
          <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-[#545f72] uppercase tracking-widest">Methodology Pipeline</div>
          <NavLink to="/dataset-eda" icon={Database} onClick={() => setIsOpen(false)}>Dataset & EDA</NavLink>
          <NavLink to="/feature-engineering" icon={GitBranch} onClick={() => setIsOpen(false)}>Feature Engineering</NavLink>
          <NavLink to="/model-development" icon={CheckCircle} onClick={() => setIsOpen(false)}>Model Development</NavLink>
          <NavLink to="/evaluation" icon={FileText} onClick={() => setIsOpen(false)}>Evaluation & Tuning</NavLink>
          <NavLink to="/notebooks" icon={BookOpen} onClick={() => setIsOpen(false)}>Jupyter Notebooks</NavLink>
          
          <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-[#545f72] uppercase tracking-widest">Live Application</div>
          <NavLink to="/classifier" icon={Search} onClick={() => setIsOpen(false)}>Live Classifier</NavLink>
          <NavLink to="/team-methodology" icon={Users} onClick={() => setIsOpen(false)}>Team & Reports</NavLink>
        </nav>
        
        <div className="p-4 m-4 rounded-xl bg-gradient-to-br from-[#0f1f3d] to-[#1a365d] border border-[#2a9d8f]/20 shadow-inner">
          <div className="text-[10px] text-[#b7c6ed] font-mono text-center tracking-wider">v1.0.0 &bull; ISOT DB SYNCED</div>
        </div>
      </div>
    </>
  );
