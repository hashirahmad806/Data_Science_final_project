...
export default function ModelDevelopment() {
  ...
  const advanced = [
    ...
    },
    {
      ...
      config:
        "LinearSVC (C=1.0, max_iter=5000) wrapped in CalibratedClassifierCV",
      badge: "Phase3_Advanced.ipynb",
      cell: "Cell 12",
    },
    {
    ...
    <div className="space-y-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-secondary max-w-3xl leading-relaxed">
            Phase 3 explores a range of machine learning classifiers. Starting
            from baseline linear/probabilistic models, we move to advanced
            ensemble architectures and support vector machines to achieve robust
            classification.
          </p>
        </div>
        <Link
      ...
      <section>
        <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <Layers size={18} className="text-primary" /> Training Input
          Dimensions
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-xl bg-surface-container border border-surface-variant flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-secondary bg-surface-variant px-2.5 py-1 rounded">
              80% Split
            </span>
          </div>
          <div className="p-4 rounded-xl bg-surface-container border border-surface-variant flex items-center justify-between">
          ...
          <div className="p-4 rounded-xl bg-surface-container border border-surface-variant flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-secondary bg-[#E6F4F1] text-success px-2.5 py-1 rounded font-bold">
              Optimized
            </span>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="mt-4 p-3 bg-surface-container rounded-lg border border-surface-variant flex gap-2 items-start">
                    ...
                    <div>
                      ...
                      <code className="text-[10px] font-mono text-primary block mt-0.5">
                        {b.config}
                      </code>
                    </div>
                  </div>
      ...
      <section className="space-y-6">
        <h3 className="text-xl font-semibold border-b border-surface-variant pb-2 flex items-center gap-2">
          <Cpu size={20} className="text-primary" /> Advanced ML Models (Phase
          3)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {advanced.map((b) => {
            ...
            return (
              <Card
                key={b.name}
                className={`flex flex-col justify-between hover:shadow-md transition-shadow ${isBest ? "border-primary shadow-sm bg-primary/5" : ""}`}
              >
                <div>
                  <CardHeader
                    title={b.name}
      ...
      <section>
        <Card className="bg-primary/5 border-primary/20">
          <CardHeader
            ...
            rightElement={
              <BADGE notebook="Phase3_Advanced.ipynb" cell="Cell 45" />
            }
          />
          <div className="flex gap-4 mt-4 p-4 bg-white border border-outline-variant rounded-xl flex-col md:flex-row items-center md:items-start">
            ...
            <div>
              <p className="text-base font-bold text-primary mb-1">
                Random Forest (Tuned) — 99.82% F1 Score
              </p>
              <p className="text-sm text-secondary leading-relaxed mb-3">
                After comprehensive comparison, the{" "}
                <strong>Tuned Random Forest Classifier</strong> was chosen for
                production deployment. It achieves a near-perfect balance,
                missing only 21 articles out of 7,821 test samples (14 False
                Negatives, 7 False Positives).
              </p>
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs font-mono text-secondary">
                <li className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-[#2a9d8f] rounded-full inline-block" />{" "}
                  Accuracy: 99.82%
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-[#2a9d8f] rounded-full inline-block" />{" "}
