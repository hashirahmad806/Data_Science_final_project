...
import { ArrowLeft, BookOpen, Download, ExternalLink, Code } from 'lucide-react';
...
export default function Notebooks() {
      ...
      <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <span className="text-sm font-semibold text-primary font-mono">{current?.filename}</span>
            <a 
              href={`/api/notebooks/${activeNotebook}/download`}
        ...
        <div className="flex-1 bg-white border border-surface-variant rounded-lg overflow-hidden shadow-sm relative">
          <iframe 
            src={`/api/notebooks/${activeNotebook}/preview`} 
  ...
  return (
    <div className="space-y-12">
      <div>
        <h2 className="text-3xl font-bold text-primary mb-2">Project Notebooks</h2>
        <p className="text-secondary max-w-3xl">
          Verifiable proof of the Data Science pipeline. Inspect the raw code, data transformations, 
          and training pipelines actually used in the capstone project.
        </p>
      </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {notebooks.map(nb => (
            <Card key={nb.id} className="flex flex-col justify-between h-full hover:shadow-lg transition-all duration-300">
              <div>
                ...
                <h4 className="text-lg font-bold text-primary mb-2">{nb.title}</h4>
                <p className="text-sm text-secondary mb-4 leading-relaxed">{nb.description}</p>
                
                <div className="flex flex-wrap gap-1.5 mb-6">
                  {nb.libraries.map(lib => (
                    <span key={lib} className="text-xs font-mono bg-surface-variant text-on-surface px-2 py-0.5 rounded">
                      {lib}
                    </span>
                  ))}
                </div>
              ...
              <div className="border-t border-surface-variant pt-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <a 
                    href={`/api/notebooks/${nb.id}/download`}
      ...
      <section>
        <h3 className="text-xl font-semibold mb-6">Evaluation Rubric Coverage</h3>
          ...
          <table className="w-full text-sm">
            <thead className="bg-surface-variant text-on-surface">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-semibold">Addressed In</th>
                <th className="text-center px-4 py-3 font-semibold">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-surface-variant">
              {[
                { criterion: "Problem Definition & EDA", weight: "20%", notebook: "Phase 1 (57 cells, 10+ plots)", status: "Complete" },
                { criterion: "Feature Engineering & Selection", weight: "20%", notebook: "Phase 2 (TF-IDF, Chi-Sq, RF Importance)", status: "Complete" },
                { criterion: "Model Development", weight: "25%", notebook: "Phase 3 (RF, XGBoost, SVM, LogReg)", status: "Complete" },
                { criterion: "Model Evaluation & Optimization", weight: "20%", notebook: "Phase 3 (GridSearchCV, ROC, Confusion Matrix)", status: "Complete" },
                { criterion: "Report & Presentation", weight: "15%", notebook: "README.md + Data Dictionary", status: "Complete" },
              ].map((row) => (
                <tr key={row.criterion} className="hover:bg-surface-container/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-primary">{row.criterion}</td>
                  <td className="px-4 py-3 text-center font-mono text-secondary">{row.weight}</td>
